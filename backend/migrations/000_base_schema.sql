...
-- Description: Creates core tables (tenants, users, user_profiles, loads) that all
-- subsequent migrations depend on. These were previously created by TypeORM synchronize.
...
DO $$ BEGIN
  CREATE TYPE tenant_status AS ENUM ('ACTIVE', 'SUSPENDED', 'PENDING_ACTIVATION', 'DEACTIVATED');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE tenant_type AS ENUM ('ENTERPRISE', 'SMALL_BUSINESS', 'INDIVIDUAL', 'PARTNER');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE tenant_kyc_status AS ENUM ('PENDING', 'SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'INCOMPLETE');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE TYPE kyc_requirement_level AS ENUM ('BASIC', 'STANDARD', 'ENHANCED', 'PREMIUM');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE TYPE load_type AS ENUM ('FTL', 'LTL', 'REEFER', 'FLATBED', 'TANKER', 'INTERMODAL', 'OTHER');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE equipment_type AS ENUM ('DRY_VAN', 'REEFER', 'FLATBED', 'TANKER', 'CONTAINER', 'OTHER');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE cargo_type AS ENUM ('GENERAL', 'FRAGILE', 'HAZARDOUS', 'REFRIGERATED', 'LIQUID', 'OVERSIZED', 'VALUABLE');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE TYPE payment_terms AS ENUM ('Prepaid', 'OnDelivery', 'Net15', 'Net30', 'Net45', 'Net60');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE packaging_type AS ENUM ('Palletized', 'Loose', 'Containerized', 'Crate', 'Drum', 'Other');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- Self-referential FK for createdByCargoOwnerId
ALTER TABLE users
  ADD CONSTRAINT fk_users_created_by
  FOREIGN KEY ("createdByCargoOwnerId") REFERENCES users(id) ON DELETE SET NULL
  NOT VALID; -- NOT VALID avoids scanning existing rows on first run
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_rating ON user_profiles(rating, "totalTrips");
...
CREATE INDEX IF NOT EXISTS idx_loads_tenant_status_owner ON loads("tenantId", status, "cargoOwnerId");
CREATE INDEX IF NOT EXISTS idx_loads_status_dates ON loads(status, "pickupDate", "deliveryDate");
CREATE INDEX IF NOT EXISTS idx_loads_cargo_urgency ON loads("cargoType", "urgencyLevel");
