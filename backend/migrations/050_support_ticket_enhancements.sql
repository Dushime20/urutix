...
-- ── 2. Add new columns to disputes_v2 ────────────────────────
...
-- ── 3. Unique index for ticket_number ────────────────────────

CREATE UNIQUE INDEX IF NOT EXISTS idx_disputes_v2_ticket_number
  ON disputes_v2 (ticket_number)
  WHERE ticket_number IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_disputes_v2_assigned_to
  ON disputes_v2 (assigned_to_user_id, status);

CREATE INDEX IF NOT EXISTS idx_disputes_v2_sla_first_response
  ON disputes_v2 (sla_first_response_due)
  WHERE sla_first_response_breached = FALSE AND first_response_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_disputes_v2_sla_resolution
  ON disputes_v2 (sla_resolution_due)
...
COMMENT ON COLUMN disputes_v2.ticket_number IS 'Human-readable ticket ID: SUP-2026-000001';
