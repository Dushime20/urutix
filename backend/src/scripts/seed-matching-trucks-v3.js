...
async function seedTrucks() {
  try {
    ...
    const trucks = [
        {
            plate: 'LA-TRUCK-01',
            lat: 34.0500, lng: -118.2400, // LA
            type: 'VAN'
        },
        {
            plate: 'CHI-TRUCK-01',
            lat: 41.8700, lng: -87.6200, // Chicago
            type: 'VAN'
        },
        {
            plate: 'FAR-TRUCK-01',
    ...
    for (const t of trucks) {
        ...
        // Pass lng, lat to ST_MakePoint (Order: X=lng, Y=lat)
        await pool.query(`
            INSERT INTO trucks (
                ...
                $1, $2, $3, $4,
                $5, 20000, 100,
                'AVAILABLE', true, ST_SetSRID(ST_MakePoint($6, $7), 4326),
                NOW(), NOW()
            )
        `, [truckId, transporterId, tenantId, t.plate, t.type, t.lng, t.lat]);
