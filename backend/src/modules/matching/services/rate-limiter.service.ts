...
export class RateLimiterService {
  ...
  private readonly inMemoryLimits = new Map<string, { count: number; resetTime: number }>();
    ...
    try {
      ...
      if (inMemoryLimit) {
        if (now > inMemoryLimit.resetTime) {
          // Reset window
          this.inMemoryLimits.set(key, { count: 1, resetTime: now + config.windowMs });
   ...
   */
  private async checkDatabaseLimit(tenantId: string, config: RateLimitConfig): Promise<void> {
    try {
      const now = new Date();
   ...
   */
  async getCurrentUsage(tenantId: string): Promise<{ current: number; limit: number; resetTime: number }> {
    const config = this.getRateLimitConfig(tenantId);
    const key = `matching:${tenantId}`;
  ...
  async cleanupExpiredRecords(): Promise<void> {
    try {
      ...
      if (deletedCount.affected > 0) {
        this.logger.log(`Cleaned up ${deletedCount.affected} expired rate limit records`);
      }
    } catch (error) {
      this.logger.warn(`Failed to cleanup expired rate limit records: ${error.message}`);
    }
  }
}
