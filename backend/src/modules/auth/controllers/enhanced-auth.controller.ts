...
export class EnhancedAuthController {
  ...
  @Get('tenants')
  @ApiOperation({
    summary: 'Get active tenants for signup',
    description: 'Public endpoint to fetch active tenants that users can sign up to join',
  })
  @ApiOkResponse({
  ...
  async getActiveTenantsForSignup(): Promise<ApiResponseDto> {
    try {
      this.logger.log('Fetching active tenants for signup');

      const tenants = await this.authService.getActiveTenantsForSignup();

      this.logger.log(`Retrieved ${tenants.length} active tenants for signup`);
      return {
        success: true,
