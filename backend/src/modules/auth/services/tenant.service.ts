...
export class TenantService {
  ...
  async createTenant(createTenantDto: any): Promise<Tenant> {
    ...
    this.logger.log(`G�� Tenant created with ID: ${finalTenant.id}`);
    ...
    try {
      ...
      if (tenantAdminUser) {
        ...
        this.logger.log(`G�� Tenant admin user created with ID: ${tenantAdminUser.id}`);
      ...
      this.logger.error(`G�� Error creating tenant admin user: ${error.message}`);
      this.logger.error(`G�� Error stack: ${error.stack}`);
      if (error.code) {
        this.logger.error(`G�� Error code: ${error.code}`);
      }
  ...
  private async sendTenantPasswordSetupEmail(
    ...
    try {
      ...
      if (!email || email.trim() === '') {
        this.logger.error('G�� Email address is empty! Cannot send email.');
        return;
      }
      ...
      try {
        if (!this.emailService) {
          this.logger.error('G�� EmailService is not injected properly!');
          this.logger.warn('G��n+� EmailService is not available, skipping email send');
          return;
        }
        ...
        this.logger.log(`G�� Password setup email sent successfully to: ${email}`);
        this.logger.log(`G�� Check the inbox (and spam folder) for: ${email}`);
      } catch (emailError: any) {
        this.logger.error(`G�� Failed to send password setup email: ${emailError.message}`);
        this.logger.error(`G�� Error stack: ${emailError.stack}`);
        if (emailError.code) {
          this.logger.error(`G�� Error code: ${emailError.code}`);
        }
        if (emailError.response) {
          this.logger.error(`G�� Error response: ${emailError.response}`);
        }
        if (emailError.responseCode) {
          this.logger.error(`G�� Error response code: ${emailError.responseCode}`);
        }
        if (emailError.command) {
          this.logger.error(`G�� Failed command: ${emailError.command}`);
        }
        this.logger.error(`G�� Full email error:`, JSON.stringify(emailError, Object.getOwnPropertyNames(emailError)));
        // Don't throw - tenant creation should succeed even if email fails
      }

      this.logger.log('=��� ========== TENANT EMAIL SENDING PROCESS END ==========');
    } catch (error: any) {
      this.logger.error(`G�� Error in sendTenantPasswordSetupEmail: ${error.message}`);
      this.logger.error(`G�� Error stack: ${error.stack}`);
      // Don't throw - tenant creation should succeed even if email fails
    }
  }
  ...
  async getSearchedTenants(query: FindTenantsDto) {
    ...
    console.log('G�� [SERVICE] Found tenants:', tenants.length, 'out of', total);
    console.log('G�� [SERVICE] All tenants from DB (no filters):', JSON.stringify(tenants, null, 2));
    console.log('G�� [SERVICE] Tenant names:', tenants.map(t => t.name));
    console.log('G�� [SERVICE] Tenant statuses:', tenants.map(t => ({
      name: t.name,
    ...
    console.log('G�� [SERVICE] Paginated response:', JSON.stringify(response, null, 2));
    return response;
  }
