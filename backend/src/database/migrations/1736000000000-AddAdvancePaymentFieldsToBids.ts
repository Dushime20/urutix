  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Check if advancePaymentPercentage column exists
    const advancePaymentPercentageExists = await queryRunner.query(`
      SELECT column_name 
      FROM information_schema.columns 
      WHERE table_name = 'bids' AND column_name = 'advancePaymentPercentage'
    `);

    if (advancePaymentPercentageExists.length === 0) {
    ...
    // Check if requireAdvancePayment column exists
    const requireAdvancePaymentExists = await queryRunner.query(`
      SELECT column_name 
      FROM information_schema.columns 
      WHERE table_name = 'bids' AND column_name = 'requireAdvancePayment'
    `);

    if (requireAdvancePaymentExists.length === 0) {
