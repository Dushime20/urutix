    ...
    <div className="max-w-[1400px] mx-auto space-y-12 animate-fade-in pb-24">
      ...
      <div className="relative overflow-hidden bg-slate-900 rounded-[2rem] p-6 text-white shadow-2xl flex items-center justify-between group">
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight leading-none mb-1">Routing</h1>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 p-8 shadow-sm space-y-8 group overflow-hidden relative">
        <div className="flex flex-col md:flex-row gap-6 items-end">
           <div className="flex-1 space-y-3">
              ...
              <input type="text" placeholder="Scan Load Node..." value={selectedLoadId} onChange={e => setSelectedLoadId(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-8 py-5 text-sm font-bold uppercase text-slate-900 transition-all focus:bg-white focus:border-primary-600 outline-none" />
           </div>
           <button onClick={handleGetMultiStop} className="px-10 py-5 bg-slate-900 text-white rounded-2xl text-sm font-bold uppercase flex items-center gap-3 hover:bg-primary-600 shadow-xl transition-all">
                  ...
                  <div className="space-y-10 pl-4">
                    {multiStop.stops.sort((a,b) => a.sequence - b.sequence).map((stop, idx) => (
                      <div key={stop.stopId} className="relative flex gap-8">
                         ...
                         <div className="w-12 h-12 rounded-2xl bg-slate-900 text-white flex items-center justify-center font-bold text-xs shadow-xl shrink-0 italic">{stop.sequence}</div>
                         <div className="flex-1 space-y-3 pt-1">
                            <div className="flex items-center justify-between">
                               <div className="flex items-center gap-3">
                                  <MapPin size={16} className="text-primary-600" />
                                  <h4 className="text-sm font-bold text-slate-900 uppercase tracking-tight italic">{stop.location.name}</h4>
                               </div>
            ...
            <div className="lg:col-span-4 space-y-8">
               {multiStop.optimizedRoute && (
                  <div className="bg-slate-900 rounded-[3rem] p-12 text-white shadow-2xl space-y-10 relative overflow-hidden group">
                     ...
                     <h3 className="text-sm font-bold uppercase text-slate-500 italic">Trajectory</h3>
                     <div className="space-y-8">
                        <div>
                           <p className="text-xs font-bold text-slate-500 uppercase mb-2">Total Field</p>
                        ...
                        <div className="pt-8 border-t border-white/5">
                           <p className="text-xs font-bold text-slate-500 uppercase mb-2">Total Interval</p>
      ...
      {showCreateModal && (
        <Dialog open={showCreateModal} onClose={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-[4rem] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-slide-up">
             ...
             <form onSubmit={(e) => { e.preventDefault(); /* handle sequence logic */ }} className="p-16 space-y-10">
                <div className="space-y-10">
                   <div className="flex justify-between items-center">
                      <h3 className="text-sm font-bold text-slate-900 uppercase">Node Registry</h3>
                      <button type="button" className="px-6 py-2 bg-slate-900 text-white text-xs font-bold uppercase rounded-xl">Add Node</button>
                   </div>
                   {/* Simplified for now as it's a sub-component simulation */}
                   <p className="text-sm font-bold text-slate-400 uppercase italic text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">Terminal Node Configuration Grid</p>
                </div>

                <div className="pt-8 flex gap-4">
                   <button type="button" onClick={() => setShowCreateModal(false)} className="px-10 py-5 bg-slate-50 text-sm font-bold uppercase text-slate-400 rounded-2xl">Abort</button>
