  ...
  if (!load) {
    return (
      <div className="max-w-[800px] mx-auto p-12 bg-white rounded-[3.5rem] border border-slate-100 shadow-sm text-center space-y-8 mt-24">
        ...
        <h2 className="text-2xl font-bold text-slate-900 uppercase italic">Target Not Found</h2>
        <button onClick={() => navigate('/dashboard/broker/loads')} className="px-12 py-5 bg-slate-900 text-white rounded-2xl text-sm font-bold uppercase shadow-xl flex items-center justify-center gap-3 mx-auto transition-all hover:bg-primary-600">
    ...
    <div className="max-w-[1400px] mx-auto space-y-12 animate-fade-in pb-32">
      ...
      <div className="relative overflow-hidden bg-slate-900 rounded-[2rem] p-6 text-white shadow-2xl flex items-center justify-between group">
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight leading-none mb-1">{load.title}</h1>
        ...
        <div className="relative z-10 flex items-center gap-12 mr-4 text-right">
           <div className="text-center hidden md:block">
             ...
             <p className="text-xs font-bold text-slate-500 uppercase mt-0.5">Yield</p>
           </div>
           {load.brokerCommissionAmount && (
             <div className="text-center hidden md:block">
               ...
               <p className="text-xs font-bold text-slate-500 uppercase mt-0.5">Profit</p>
             </div>
           )}
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-6 bg-slate-50/50 rounded-2xl border border-slate-50 group-hover:bg-white transition-all">
                    ...
                    <span className="text-sm font-bold text-slate-900 uppercase">{spec.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-sm p-12 space-y-10 group relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div onClick={() => setShowMatchModal(true)} className="p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 cursor-pointer group/card hover:bg-white hover:shadow-2xl transition-all duration-500 overflow-hidden relative">
                ...
                <h4 className="text-xl font-bold text-slate-900 mb-2 uppercase tracking-tight italic">Direct Match</h4>
              ...
              <div onClick={() => setShowAuctionModal(true)} className="p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 cursor-pointer group/card hover:bg-white hover:shadow-2xl transition-all duration-500 overflow-hidden relative">
                ...
                <h4 className="text-xl font-bold text-slate-900 mb-2 uppercase tracking-tight italic">Auction</h4>
        ...
        <div className="lg:col-span-4 space-y-12">
            ...
            <div className="space-y-10 relative">
               ...
               <div className="flex gap-6 group/point z-10 relative">
                  ...
                  <div>
                     ...
                     <p className="text-sm font-bold text-slate-900 leading-snug italic uppercase">{load.pickupLocation}</p>
                  </div>
               </div>
               <div className="flex gap-6 group/point z-10 relative">
                  <div className="w-7 h-7 bg-slate-900 rounded-lg flex items-center justify-center text-white text-sm font-bold shadow-lg">B</div>
                  <div>
                     <p className="text-xs font-bold text-slate-400 uppercase mb-1">Target</p>
                     <p className="text-sm font-bold text-slate-900 leading-snug italic uppercase">{load.deliveryLocation}</p>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10 flex items-center justify-between mb-10">
              <h3 className="text-sm font-bold uppercase text-slate-500 italic">Agreements</h3>
            ...
            {contract ? (
              <div className="space-y-8 relative z-10">
                ...
                <button onClick={() => navigate('/dashboard/broker/contracts')} className="w-full py-5 bg-white text-slate-900 rounded-2xl text-sm font-bold uppercase hover:bg-primary-600 hover:text-white transition-all">View Record <ExternalLink size={14} className="inline ml-2" /></button>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm p-10 space-y-10 group transition-all duration-500">
             <h3 className="text-sm font-bold text-slate-900 uppercase">Shipper</h3>
             <div className="flex items-center gap-6 p-6 bg-slate-50 rounded-[2.5rem] border border-slate-50 hover:bg-white hover:shadow-2xl transition-all">
                ...
                <div>
                  <h4 className="text-lg font-bold text-slate-900 uppercase italic">{load.cargoOwner?.profile?.firstName || 'Shipper'}</h4>
      ...
      <Dialog open={showContactModal} onOpenChange={setShowContactModal}>
        <DialogContent className="sm:max-w-xl bg-white rounded-[4rem] border-none shadow-2xl p-0 overflow-hidden animate-slide-up">
           <div className="p-16 bg-slate-900 text-white relative overflow-hidden">
              ...
              <p className="text-sm font-bold text-slate-500 uppercase mt-6 italic">Authorization Interface</p>
           ...
           <div className="p-16 space-y-12">
              <div className="flex items-center gap-10 p-10 bg-slate-50 rounded-[3rem] border border-slate-100 shadow-sm">
                 ...
                 <div className="space-y-2">
                    <h3 className="text-3xl font-bold text-slate-900 uppercase italic leading-none">{load.cargoOwner?.profile?.firstName || 'Shipper'} {load.cargoOwner?.profile?.lastName || 'Name'}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <a href={`mailto:${load.cargoOwner?.email}`} className="flex flex-col p-10 bg-slate-50 rounded-[2.5rem] hover:bg-white hover:shadow-2xl hover:scale-105 transition-all group/li border border-transparent hover:border-slate-100">
                   ...
                   <p className="text-sm font-bold text-slate-900 truncate tracking-tight">{load.cargoOwner?.email || 'Awaiting...'}</p>
                </a>
                {load.cargoOwner?.phone && (
                   <a href={`tel:${load.cargoOwner.phone}`} className="flex flex-col p-10 bg-slate-50 rounded-[2.5rem] hover:bg-white hover:shadow-2xl hover:scale-105 transition-all group/li border border-transparent hover:border-slate-100">
                      ...
                      <p className="text-sm font-bold text-slate-900 tracking-tight">{load.cargoOwner.phone}</p>
                   </a>
                )}
              </div>
              <button onClick={() => setShowContactModal(false)} className="w-full py-7 bg-slate-900 text-white rounded-[2rem] text-sm font-bold uppercase hover:bg-primary-600 transition-all shadow-2xl">Abort Interface</button>
           </div>
        </DialogContent>
      </Dialog>
    </div>
