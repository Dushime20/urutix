    ...
    <div className="max-w-[1440px] mx-auto space-y-12 animate-fade-in pb-24">
      ...
      <div className="relative overflow-hidden bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between group">
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight leading-none mb-1 text-slate-900 dark:text-white">Market Prices</h1>
        ...
        <div className="relative z-10 hidden md:flex items-center gap-6 mr-4">
          <div className="text-center">
            ...
            <p className="text-[8px] font-bold text-slate-500 uppercase mt-0.5 dark:text-slate-400">Currency</p>
          </div>
          <div className="text-center">
            <p className="text-xl font-bold leading-none text-emerald-500">Live</p>
            <p className="text-[8px] font-bold text-slate-500 uppercase mt-0.5 dark:text-slate-400">Status</p>
          </div>
        </div>
      </div>

      {/* Route Analysis Terminal */}
      <div className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm relative group overflow-hidden dark:bg-slate-900 dark:border-slate-800">
        ...
        <div className="relative z-10 space-y-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-slate-900 uppercase dark:text-white">Route Details</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-16 pr-8 py-5 text-sm font-bold tracking-tight text-slate-900 focus:bg-white focus:border-primary-600 focus:ring-0 transition-all outline-none dark:bg-slate-800/50 dark:text-white dark:border-slate-800"
            ...
            <div className="space-y-3">
              ...
              <div className="relative group/input">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-8 py-5 text-sm font-bold tracking-tight text-slate-900 focus:bg-white focus:border-primary-600 focus:ring-0 transition-all outline-none dark:bg-slate-800/50 dark:text-white dark:border-slate-800"
                  placeholder="KM"
                />
              </div>
            </div>
            <div className="flex items-end">
              <button
                ...
                disabled={loading}
                className="w-full bg-slate-900 text-white rounded-2xl h-[60px] text-sm font-bold uppercase tracking-[0.2em] shadow-xl hover:-translate-y-1 hover:bg-primary-600 transition-all active:translate-y-0 flex items-center justify-center gap-4 dark:bg-slate-950"
              >
          ...
          {marketData.rateRecommendations && (
            <div className="bg-white rounded-[3rem] border border-slate-100 p-12 shadow-sm space-y-12 dark:bg-slate-900 dark:border-slate-800">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <p className="text-sm font-bold text-slate-400 uppercase border border-slate-100 px-4 py-2 rounded-xl dark:border-slate-800">Price Suggestions</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                {[
                  ...
                  <div key={idx} className="group p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:shadow-2xl transition-all duration-500 dark:bg-slate-800/50 dark:border-slate-800">
                    ...
                    <h4 className="text-4xl font-bold text-slate-900 mb-2 dark:text-white">{rec.value.toLocaleString()} <span className="text-xs italic uppercase text-slate-300">KES</span></h4>
                    <p className="text-sm font-bold text-primary-600 uppercase mb-6">{rec.meta}</p>
                    <p className="text-xs font-medium text-slate-500 leading-relaxed dark:text-slate-400">{rec.desc}</p>
                  </div>
                ))}
              </div>
              
              <div className="p-10 bg-slate-900 rounded-[2.5rem] relative overflow-hidden group dark:bg-slate-950">
          ...
          {marketData.demandForecast && (
            <div className="bg-white rounded-[3.5rem] border border-slate-100 p-12 shadow-sm space-y-12 dark:bg-slate-900 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col justify-between dark:bg-slate-800/50 dark:border-slate-800">
                  <p className="text-sm font-bold text-slate-400 uppercase mb-8">Short-term Lift (7D)</p>
                  <p className="text-4xl font-bold text-slate-900 dark:text-white">{marketData.demandForecast.next7Days} <span className="text-sm uppercase text-slate-300">Records</span></p>
                </div>
                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col justify-between dark:bg-slate-800/50 dark:border-slate-800">
                  <p className="text-sm font-bold text-slate-400 uppercase mb-8">Terminal Run (30D)</p>
                  <p className="text-4xl font-bold text-slate-900 dark:text-white">{marketData.demandForecast.next30Days} <span className="text-sm uppercase text-slate-300">Records</span></p>
                </div>
                <div className="md:col-span-2 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-6 dark:bg-slate-800/50 dark:border-slate-800">
                  ...
                  <div className="w-full h-3 bg-white rounded-full border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
                    <div 
    ...
    <div className="group/chart space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          <div className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl dark:bg-slate-800/50 dark:border-slate-800">
            <span className="text-xs font-bold text-slate-400 uppercase">Low:</span>
            <span className="text-sm font-bold text-slate-900 ml-2 dark:text-white">{min.toLocaleString()}</span>
          </div>
          <div className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl dark:bg-slate-800/50 dark:border-slate-800">
            <span className="text-xs font-bold text-slate-400 uppercase">Peak:</span>
            <span className="text-sm font-bold text-slate-900 ml-2 dark:text-white">{max.toLocaleString()}</span>
          </div>
        </div>
      </div>
        ...
        {data.map((value, idx) => {
          ...
          return (
            <div key={idx} className="flex-1 group/bar relative">
              ...
              >
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 opacity-0 group-hover/bar:opacity-100 transition-opacity bg-slate-900 text-white text-xs font-bold px-3 py-2 rounded-xl pointer-events-none uppercase z-10 dark:bg-slate-950">
      ...
      <div className="flex justify-between items-center text-xs font-bold text-slate-200 uppercase pt-4 border-t border-slate-50 dark:border-slate-800/50">
        <span>Min</span>
