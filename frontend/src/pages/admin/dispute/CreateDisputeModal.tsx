...
const CreateDisputeModal: React.FC<Props> = ({ onClose, onCreated }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[24px] w-full max-w-lg border border-gray-100 dark:border-slate-800">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-red-50 rounded-xl flex items-center justify-center">
              <Gavel className="w-4 h-4 text-red-500" />
            </div>
            <h3 className="text-base font-black text-gray-900 dark:text-white">Raise a Dispute</h3>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input value={form.title} onChange={set('title')} placeholder="Brief title of the dispute"
              className="w-full px-3 py-2 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-[#2c5173] focus:border-transparent" />
          </div>
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-800 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl text-sm font-bold text-gray-600 dark:text-slate-300 hover:bg-gray-100 transition-colors">Cancel</button>
