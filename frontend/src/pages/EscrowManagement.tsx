  ...
  const columns: Column<EscrowAccount>[] = useMemo(() => [
    {
      ...
      render: (_v, account) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-900 dark:text-white">{account.id}</span>
        </div>
      ),
    },
    {
      ...
      render: (_v, account) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-gray-600 dark:text-slate-300 font-mono">{account.tripId}</span>
        </div>
      ),
    },
    {
      ...
      render: (_v, account) => (
        <div className="text-sm space-y-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-gray-900 dark:text-white font-bold text-xs">{account.cargoOwner}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Truck className="w-3 h-3 text-emerald-500" />
            <span className="text-gray-600 dark:text-slate-300 font-medium text-xs">{account.truckOwner}</span>
          </div>
        </div>
      ),
    },
    {
      ...
      sortable: true,
      render: (_v, account) => (
        <span className="text-sm font-black text-gray-900 dark:text-white">{fmtFull(account.amount)}</span>
      ),
    },
    {
      key: 'status',
  ...
  return (
    <AdminPageLayout
      ...
      actions={
        <button
          onClick={fetchData}
          className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-slate-800 transition-all duration-200 text-sm font-bold"
        >
    ...
    >
      {loading ? (
        <div className="flex items-center justify-center h-64 gap-3">
          ...
          <span className="text-gray-600 dark:text-slate-300 font-medium">Loading escrow data...</span>
        </div>
      ) : error ? (
      ...
      {showDetailsModal && selectedEscrow && (        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-[24px] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto pb-24 lg:pb-8">
            <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white"><TranslatedText text="Escrow Details" /></h2>
                  <p className="text-xs text-gray-500 font-medium mt-0.5">{selectedEscrow.id}</p>
                </div>
              </div>
              <button
                onClick={() => setShowDetailsModal(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600 dark:text-slate-300 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-8">
              {/* Status Banner */}
              <div className={`p-4 rounded-xl border ${selectedEscrow.status === 'DISPUTED' ? 'bg-red-50 border-red-100' : 'bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-slate-800'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${selectedEscrow.status === 'DISPUTED' ? 'bg-red-100 text-red-600' : 'bg-white dark:bg-slate-900 text-gray-600 dark:text-slate-300'}`}>
                  ...
                  <div>
                    ...
                    <span className="text-sm font-bold text-gray-900 dark:text-white">{selectedEscrow.status}</span>
                  </div>
                </div>
              ...
              {/* Amount */}
              <div className="bg-gray-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-gray-100 dark:border-slate-800 text-center">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="font-bold text-gray-900 dark:text-white font-mono text-sm bg-gray-50 dark:bg-slate-800/50 px-3 py-2 rounded-lg inline-block border border-gray-100 dark:border-slate-800">
                ...
                <div className="col-span-2">
                  ...
                  <div className="font-medium text-gray-700 dark:text-slate-300 text-sm bg-gray-50 dark:bg-slate-800/50 p-3 rounded-xl border border-gray-100 dark:border-slate-800">
              ...
              {selectedEscrow.status === 'ACTIVE' && (
                <div className="flex gap-3 pt-6 border-t border-gray-100 dark:border-slate-800">
                  ...
                  <button className="flex-1 bg-white dark:bg-slate-900 text-red-600 border border-red-100 py-3 rounded-xl hover:bg-red-50 transition-all font-bold text-sm flex items-center justify-center gap-2">
