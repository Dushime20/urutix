...
const CustomsOfficerPasswordSetup = () => {
  ...
  const onSubmit = async (data: PasswordSetupFormData) => {
    ...
    try {
      setIsLoading(true);
      await authAPI.setupCustomsOfficerPassword({ token, password: data.password, confirmPassword: data.confirmPassword });
      setIsSuccess(true);
...
export default CustomsOfficerPasswordSetup;
