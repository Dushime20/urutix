  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 sm:p-8 md:p-12 animate-in fade-in duration-700">
      <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
        {/* Futuristic Header */}
        <div className="relative bg-white rounded-[2rem] sm:rounded-[40px] border border-slate-100 p-8 sm:p-12 overflow-hidden shadow-sm group">
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight mb-4 sm:mb-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl sm:rounded-3xl py-4 sm:py-6 pl-12 sm:pl-16 pr-6 sm:pr-8 text-xs sm:text-base text-slate-900 font-bold placeholder:text-slate-300 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-[#345E85] outline-none transition-all shadow-inner"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          {[
            ...
            <div key={idx} className="bg-white p-6 sm:p-8 rounded-3xl sm:rounded-[32px] border border-slate-100 shadow-sm hover:shadow-md transition-all group cursor-pointer">
              <div className="flex items-center gap-4 mb-4 sm:mb-6">
                <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-[#345E85] group-hover:text-white transition-all`}>
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-bold text-slate-900 line-clamp-1">{vector.detail}</p>
                </div>
              </div>
        ...
        {/* Knowledge Base */}
        <div className="bg-white p-6 sm:p-10 rounded-3xl sm:rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between mb-8 sm:mb-10 pb-4 sm:pb-6 border-b border-slate-50">
            <div>
              <h3 className="text-lg sm:text-xl font-black text-slate-900 uppercase tracking-tight">Intelligence Repository</h3>
          ...
          <div className="space-y-3 sm:space-y-4">
            {filteredFAQs.map(faq => (
              <div
                key={faq.id}
                className={`border rounded-2xl sm:rounded-3xl transition-all duration-300 ${expandedFAQ === faq.id ? 'border-[#345E85] bg-blue-50/20' : 'border-slate-100 bg-white hover:border-slate-200'
                  }`}
              >
                <button
                  ...
                  {expandedFAQ === faq.id && (
                    ...
                    >
                      <div className="px-6 sm:px-8 pb-6 sm:pb-8 pt-2">
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed italic border-l-4 border-[#345E85] pl-4 sm:pl-6">{faq.answer}</p>
