...
const LoansDashboard: React.FC<LoansDashboardProps> = ({ className = '' }) => {
  ...
  const loanColumns: Column<LoanRequest>[] = useMemo(() => [
    {
      key: 'id',
      label: 'Loan ID',
      render: (_: unknown, loan: LoanRequest) => (
        <>
          <div className="text-sm font-mono text-gray-900">{loan.id.slice(0, 8)}...</div>
          {loan.external_loan_ref && (
            <div className="text-xs text-gray-500">Ref: {loan.external_loan_ref}</div>
          )}
        </>
      ),
    },
    {
      ...
      render: (_: unknown, loan: LoanRequest) => (
        <>
          <div className="text-sm font-semibold text-gray-900">${loan.requested_amount.toLocaleString()}</div>
          {loan.approved_amount && loan.approved_amount !== loan.requested_amount && (
            <div className="text-xs text-green-600">Approved: ${loan.approved_amount.toLocaleString()}</div>
          )}
        </>
      ),
    },
    ...
    },
    {
      ...
      label: 'Lender',
      render: (_: unknown, loan: LoanRequest) => (
        <div className="text-sm text-gray-900">{loan.lender?.name || 'Pending Assignment'}</div>
      ),
    },
    {
      key: 'created_at',
    ...
    },
    {
      ...
      label: 'Due Date',
      render: (_: unknown, loan: LoanRequest) => (
        <span className="text-sm text-gray-500">{loan.due_date ? formatDate(loan.due_date) : 'N/A'}</span>
      ),
    },
  ], []);

  const loanRowActions: TableAction<LoanRequest>[] = useMemo(() => [
    {
      key: 'view',
      label: 'View',
      ...
      <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
        ...
        <div className="p-4">
          <StandardDataTable<LoanRequest>
            ...
            data={loans || []}
            loading={isLoading}
            error={error ? 'Failed to load loans. Please try again.' : null}
            getRowId={(row) => row.id}
            searchable={false}
            pagination
            pageSize={10}
            ...
            striped
            hoverable
            emptyMessage="You haven't requested any advances yet."
            rowActions={loanRowActions}
            ariaLabel="Loan requests"
          />
        </div>
      </div>
    </div>
