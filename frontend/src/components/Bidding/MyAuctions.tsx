                        ...
                        return (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className="w-full px-4 sm:px-6 py-4 sm:py-5 flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 text-left hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors"
                                >
                                    {/* Mobile Layout */}
                                    <div className="flex items-center gap-3 w-full sm:hidden">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase italic line-clamp-2 break-words mb-2" title={auction.load?.title || `Auction #${auction.id.slice(0, 8)}`}>
                                            ...
                                            <div className="flex flex-wrap items-center gap-1.5">
                                                ...
                                                <span className="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase bg-slate-900 dark:bg-slate-950 text-white dark:text-slate-300 border border-slate-800">
                                                    {auction.auctionType}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        {/* Mobile: Expand Icon */}
                                        {isExpanded ? (
                                            <ChevronUp size={18} className="text-slate-400 dark:text-slate-600 shrink-0" />
                                        ) : (
                                            <ChevronDown size={18} className="text-slate-400 dark:text-slate-600 shrink-0" />
                                        )}
                                    </div>

                                    {/* Mobile: Stats Row */}
                                    <div className="flex items-center justify-between w-full sm:hidden gap-2">
                                        <div className="flex items-center gap-3 text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex-wrap">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {pendingBids.length > 0 && (
                                                <span className="px-2 py-0.5 rounded-lg bg-amber-50 dark:bg-amber-900/10 text-amber-700 dark:text-amber-400 text-[9px] font-black uppercase border border-amber-200 dark:border-amber-900/30 whitespace-nowrap">
                                                    {pendingBids.length} new
                                                </span>
                                            )}
                                            <div className="text-center">
                                                <span className="text-lg font-black text-[#345E85] dark:text-blue-400">{bidCount}</span>
                                                <p className="text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Bids</p>
                                            </div>
                                        </div>
                                    </div>

                                    {/* Mobile: Action Buttons */}
                                    <div className="flex items-center gap-2 w-full sm:hidden pt-2 border-t border-slate-100 dark:border-slate-800">
                                        <button
                                            onClick={(e) => handleOpenView(auction, e)}
                                            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase tracking-wide"
                                        >
                                            <Eye size={12} />
                                            View
                                        </button>
                                        ...
                                        {auction.status !== 'CLOSED' && auction.status !== 'CANCELLED' && (
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/20 transition-colors border border-blue-100 dark:border-blue-900/30 text-[10px] font-black uppercase tracking-wide"
                                            >
                                                <Pencil size={12} />
                                                Edit
                                            </button>
                                        ...
                                        {auction.status !== 'CLOSED' && (
                                            <button
                                                ...
                                                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-500 hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors border border-red-100 dark:border-red-900/30 text-[10px] font-black uppercase tracking-wide"
                                            >
                                                <Trash2 size={12} />
                                            </button>
                                        )}
                                    ...
                                    <div className="hidden sm:flex items-center gap-4 w-full">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1 flex-wrap">
                                                <h3 className="text-base font-black text-slate-900 dark:text-slate-100 uppercase italic truncate" title={auction.load?.title || `Auction #${auction.id.slice(0, 8)}`}>
                                                    {auction.load?.title || `Auction #${auction.id.slice(0, 8)}`}
                                                </h3>
                                                <span className={cn(
                                        ...
                                        <div className="flex items-center gap-4 shrink-0">
                                            ...
                                            {auction.status !== 'CLOSED' && auction.status !== 'CANCELLED' && (
                                                <button
                                                    onClick={(e) => handleOpenEdit(auction, e)}
                                                    className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/20 transition-colors border border-blue-100 dark:border-blue-900/30 flex items-center justify-center"
                                                    title="Edit Auction"
                                                >
                                                    <Pencil size={14} />
                                                </button>
                                            ...
                                            <button
                                                onClick={(e) => handleOpenView(auction, e)}
                                                className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700 flex items-center justify-center"
                                                title="View Details"
                                            >
                                                <Eye size={14} />
                                            </button>
                                            {auction.status !== 'CLOSED' && (
                                                <button
                                                    onClick={(e) => handleDeleteAuction(auction.id, e)}
                                                    className="w-8 h-8 rounded-lg bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-500 hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors border border-red-100 dark:border-red-900/30 flex items-center justify-center"
                                                    title="Delete Auction"
                                                >
                                                    <Trash2 size={14} />
                                                </button>
