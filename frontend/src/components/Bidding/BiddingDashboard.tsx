...
const StatsCard = ({ title, value, icon: Icon, colorClass, secondaryColor }: any) => {
  ...
  return (
    <div className="flex flex-col items-center group">
      <div className="relative w-32 h-32 sm:w-44 sm:h-44 rounded-full bg-white border-[6px] sm:border-[10px] border-slate-50 flex flex-col items-center justify-center transition-all duration-500 hover:border-slate-100 hover:shadow-xl hover:shadow-slate-200/50">
        <svg className="absolute inset-0 w-full h-full -rotate-90 scale-[1.05]">
          ...
          <circle
            ...
            className={cn("sm:hidden opacity-10 transition-all duration-1000 group-hover:stroke-dashoffset-[150]", secondaryColor)}
          />
          {/* Desktop circle */}
          <circle
            cx="88"
            ...
            className={cn("hidden sm:block opacity-10 transition-all duration-1000 group-hover:stroke-dashoffset-[250]", secondaryColor)}
          />
        </svg>

        <div className={cn("p-2 sm:p-2.5 rounded-xl sm:rounded-2xl mb-1 sm:mb-2 bg-slate-50 text-slate-400 group-hover:bg-white group-hover:text-inherit transition-all duration-500 shadow-sm", colorClass)}>
          <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
        </div>

        <div className="flex flex-col items-center px-2 sm:px-4 w-full overflow-hidden">
          <span className="text-lg sm:text-2xl font-black text-[#0f172a] tracking-tight group-hover:scale-110 transition-transform duration-500 truncate w-full text-center">
            {displayValue}
          </span>
        </div>
      ...
      <div className="mt-3 sm:mt-5 text-center px-1 sm:px-2">
        <p className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest sm:tracking-[0.2em] group-hover:text-[#345E85] transition-colors duration-300 line-clamp-1">
          {title}
        </p>
      </div>
    </div>
...
const BiddingDashboard: React.FC<BiddingDashboardProps> = ({ userRole }) => {
  ...
  const renderCargoOwnerStats = () => (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 mb-8 sm:mb-12 place-items-center bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-slate-100 shadow-sm">
      <StatsCard
        title="My Auctions"
  ...
  const renderCargoOwnerTabs = () => (
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col md:flex-row gap-4 sm:gap-6 justify-between items-center bg-white p-2 sm:p-3 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-100 shadow-sm w-full overflow-hidden">
      ...
      <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-4 sm:p-8 border border-slate-100 shadow-sm min-h-[400px]">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2 sm:space-y-3">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-blue-50 flex items-center justify-center shadow-sm">
              <Shield className="w-6 h-6 sm:w-7 sm:h-7 text-[#345E85]" />
            </div>
