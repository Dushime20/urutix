...
const TenantBidding: React.FC<TenantBiddingProps> = ({ tenantId }) => {
    ...
    const bidColumns = useMemo<Column<Bid>[]>(() => [
        {
            key: 'loadId',
            label: 'Load / Lane',
            sortable: true,
        ...
        },
        {
            ...
            sortable: true,
            render: (_v, bid) => (
                <StatusBadge status={bid.status} label={<TranslatedText text={bid.status} />} />
            ),
        },
        {
            key: 'createdAt',
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[24px] border border-gray-100 dark:border-slate-800 shadow-sm min-h-[400px]">
                {isLoading ? (
                    ...
                    <StandardDataTable<Bid>
                        ...
                        searchKeys={['loadId', 'loadOrigin', 'loadDestination', 'status']}
                        stickyHeader
                        columnVisibility
                        pagination
                        emptyMessage="No Active Bids Found"
                        ariaLabel="Tenant bids"
                    />
                ) : (
