...
const RecentActivity: React.FC<RecentActivityProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'info':
        return <Info className="w-3.5 h-3.5 text-primary-600" />;
      default:
  ...
  return (
    <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-8 py-6 border-b border-gray-50 flex items-center justify-between">
        <div>
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">History</h3>
          <h4 className="text-xl font-black text-slate-800 tracking-tight">Recent Updates</h4>
        </div>
      ...
      <div className="px-2 py-4">
        {displayedActivities.length > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className="text-[10px] font-black text-primary-600 uppercase tracking-widest flex items-center hover:opacity-70 transition-opacity"
          ...
          <div className="px-8 py-12 text-center">
            ...
            <h5 className="text-sm font-black text-slate-800 uppercase tracking-tight">No activity found</h5>
            <p className="text-[13px] text-slate-400 mt-1 font-medium italic">There is no recent activity to show right now.</p>
          </div>
        )}
      </div>
      ...
      {activities.length > maxItems && (
        <div className="px-8 py-5 bg-gray-50/50 border-t border-gray-50 flex justify-center">
          <button className="text-xs font-black text-primary-600 uppercase tracking-[0.2em] hover:opacity-70 transition-opacity">
            View All History ({activities.length} entries)
          </button>
        </div>
      )}
    </div>
