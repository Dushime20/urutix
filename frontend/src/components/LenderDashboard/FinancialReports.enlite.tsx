...
const FinancialReportsEnlite: React.FC<FinancialReportsEnliteProps> = ({
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, item: GeneratedReport) => (
                <div className="min-w-0">
                    <p className="font-semibold text-slate-900 dark:text-white text-sm">{item.size}</p>
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider font-medium">
        ...
        },
        {
            ...
            render: (_: any, item: GeneratedReport) => (
                <div className="flex justify-end gap-2">
                    ...
                    <button className="p-2 text-slate-400 hover:text-slate-900 dark:text-white transition-colors" title="Email">
                        <Mail size={16} />
                    </button>
                </div>
            ),
                ...
                <DataCard
                    ...
                    actions={
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <option value="all" className="text-slate-900 dark:text-white">ALL CLASSES</option>
                                <option value="portfolio" className="text-slate-900 dark:text-white">PORTFOLIO</option>
                                <option value="financial" className="text-slate-900 dark:text-white">FINANCIAL</option>
                                <option value="risk" className="text-slate-900 dark:text-white">RISK</option>
                                <option value="compliance" className="text-slate-900 dark:text-white">COMPLIANCE</option>
                            </select>
                        </div>
                    }
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredTemplates.length === 0 ? (
                            ...
                            filteredTemplates.map(template => (
                                <div
                                    key={template.id}
                                    className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-[#2c5173] hover:shadow-lg hover:shadow-slate-100 transition-all group flex flex-col justify-between min-h-[180px]"
                                >
                                    <div>
                                        <div className="flex items-start justify-between mb-3">
                                            <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-[#2c5173] group-hover:bg-[#2c5173] group-hover:text-white transition-colors border border-slate-200 dark:border-slate-700">
                                                {template.icon}
                                            </div>
                                            <span className="text-[9px] font-bold px-2 py-0.5 rounded-lg border bg-slate-50 dark:bg-slate-800/50 text-slate-500 uppercase tracking-wider">
                                                {template.estimatedTime}
                                            </span>
                                        </div>
                                        <h5 className="font-semibold text-slate-900 dark:text-white text-sm mb-1">{template.name}</h5>
                                    ...
                                    <div className="mt-4 flex items-center justify-between gap-3">
                                        <button
                                            ...
                                            className="flex-1 py-1.5 px-3 bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 rounded-lg text-[9px] font-bold uppercase tracking-wider hover:bg-slate-100 transition-all"
                                        >
                                            View Details
                                        </button>
                                        <button
